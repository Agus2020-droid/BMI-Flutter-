// import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';

const Color primaryColor = Color(0xff0A0E21);
TextStyle labelTextStyle = const TextStyle(
  fontSize: 18,
  color: Colors.white,
);
TextStyle numberTextStyle = const TextStyle(
  fontSize: 50,
  fontWeight: FontWeight.bold,
  color: Colors.white,
);

const String underweightSevere = "Underweight (Severe thineess)";
const String underweightModerate = "Underweight (Moderate thineess)";
const String underweightMild = "Underweight (Mild thineess)";
const String normal = "Normal";
const String overweight = "Overweight";
const String obeseI = "Obese (Class I)";
const String obeseII = "Obese (Class II)";
const String obeseIII = "Obese (Class III)";
